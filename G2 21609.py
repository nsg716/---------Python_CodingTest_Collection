# 문제 6번 
# 삼성 기출문제는 요구 조건을 얼마나 구현을 잘하는지 보는 것이 핵심이다.
...
def recur(y, x):
    ...
    for k in range(4):
        ny = y + dy[k]
        nx = x + dx[k]
        if is_vaild_coord(ny,nx) and groups[ny][nx] == 0:
            if board[ny][nx] == 0:
                sz += 1
                rainbow += 1
...
def recur_remove(y, x):
    for k in range(4):
        ny = y + dy[k]
        nx = x + dx[k]
        if is_vaild_coord(ny,nx) and board[ny][nx] >= 0 and (board[ny][nx] == 0 or groups[ny][nx] == group_idx):
            board[ny][nx] = -2
            recur_remove(ny,nx)
    ...
    for i in range(N):
        for j in range(N):
            if groups[i][j] == 0:
                ...
                if board[i][j] > 0: # 블록 개수가 2보다 크거나 같은지 계산 
                    cnt = 1
                    for k in range(4):
                        ni = i + dy[k]
                        nj = j + dx[k]
                        if is_vaild_coord(ni,nj) and  (board[ni][nj] == 0 or board[ni][nj] == board[i][j]):
                            cnt += 1
                    if cnt < 2:
                        continue
